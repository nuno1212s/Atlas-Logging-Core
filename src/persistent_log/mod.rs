...
pub trait PersistentDecisionLog<RQ, OPM, POP, LS>: OrderingProtocolLog<RQ, OPM> + Send
    ...
    fn checkpoint_received(&self, mode: OperationMode, seq: SeqNo) -> atlas_common::error::Result<()>;

    /// Write a given proof to the persistent log
    fn write_proof(&self, write_mode: OperationMode, proof: PProof<RQ, OPM, POP>) -> atlas_common::error::Result<()>;

    /// Write the metadata of a decision into the persistent log
    fn write_decision_log_metadata(&self, mode: OperationMode, log_metadata: DecLogMetadata<RQ, OPM, POP, LS>) -> atlas_common::error::Result<()>;

    /// Write the decision log into the persistent log
    fn write_decision_log(&self, mode: OperationMode, log: DecLog<RQ, OPM, POP, LS>) -> atlas_common::error::Result<()>;

    /// Read a proof from the log with the given sequence number
    fn read_proof(&self, mode: OperationMode, seq: SeqNo) -> atlas_common::error::Result<Option<PProof<RQ, OPM, POP>>>;

    /// Read the decision log from the persistent storage
    fn read_decision_log(&self, mode: OperationMode) -> atlas_common::error::Result<Option<DecLog<RQ, OPM, POP, LS>>>;
    ...
    fn wait_for_full_persistence(&self, batch: BatchedDecision<RQ>, logging_decision: LoggingDecision)
